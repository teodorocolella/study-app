...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f6f5fb] bg-[radial-gradient(circle_at_top,rgba(139,92,246,0.08),transparent_55%)]">
      {/* Mobile top bar */}
      <div className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200/70 bg-white/80 px-4 py-3 backdrop-blur-md md:hidden">
        <button onClick={() => setMobileOpen(true)} aria-label="Open menu" className="text-slate-600">
        ...
        <Link to="/dashboard" className="flex items-center gap-2">
          ...
          <span className="font-display font-semibold text-slate-800">Study Hub</span>
        </Link>
        <Link to="/groups" className="relative text-slate-600" aria-label="Study Groups">
...
function Sidebar({
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-40 flex flex-col border-r border-slate-200/70 bg-white/90 backdrop-blur-md transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0 ${sidebarWidth} ${
      ...
      <div className="border-t border-slate-200/70 p-3">
        ...
        <div className={`mt-1 flex ${collapsed ? "flex-col items-center gap-1" : "items-center justify-between"}`}>
          <button
            onClick={onLogout}
            title="Log out"
            className={`flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-sm font-medium text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-700 ${
              collapsed ? "justify-center" : ""
            }`}
          >
          ...
          <button
            ...
            className="hidden rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-50 hover:text-slate-600 md:block"
          >
