...
const envSchema = z.object({
  ...
  // Email (reminders + password resets) — all optional; email is disabled
  // unless SMTP is configured. Values are trimmed because stray whitespace
  // from copy-pasting into dashboard env fields breaks SMTP auth silently.
  SMTP_HOST: z.string().trim().optional(),
  SMTP_PORT: z.coerce.number().default(587),
  SMTP_USER: z.string().trim().optional(),
  SMTP_PASS: z.string().trim().optional(),
  EMAIL_FROM: z.string().trim().optional(),
  APP_URL: z.string().trim().default("http://localhost:5173"),
